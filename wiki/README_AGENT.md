...
非边类缺口（页面内的问题，如死 state、硬编码）记入 `edges.json` 的 `issues[]`。

## 二、文件清单（谁是真源、谁是产物）

| 文件 | 角色 |
|---|---|
| `schema.md` | **契约真源**：四层模型 / 命名 / node 五节结构 / 粒度三问 / 校验规则 A~E / 变更记录。改结构规范先动它 |
| `structure.json` | **结构真源**：8 section + 15 page（`component` / `relevant_files` / `docStatus` / `nodeCount`）+ 壳层 / 弹层 / 8 个孤儿组件清单 |
| `edges.json` | **边的真源**：23 条边 + 9 条 issues |
...
**section 8 · page 15（已下钻 2 / 待铺开 13）· node 18 · 边 23（18 implemented · 4 intended · 1 undefined）· issues 9 · 正文可验证引用 231 处；`validate` 0 error / 1 warning**（唯一 warning = 13 个 page 尚未铺开的汇总提示）

- 已下钻：`page-workbench` 项目工作台（8 节点）、`page-guidance` 全链路指导工作台（10 节点）
- 待铺开（第 4 步剩余）：cockpit / milestones / screening / mentorship / supervision / mentors-pool / coach / defense / assets / knowledge-base / users / teams / login
- 3 条 high 缺口（在 `gaps.md` 有完整卡点分析）：① 工作台「完成并回写待办」未真正回写；② 「逻辑断点 → 动态待办」链断（同一根因：待办池困在组件内 state）；③ 评委提问去向待拍板
...
& $PY wiki/gen_wiki_tools.py gaps         # 重生成 gaps.md
...
**先读 skill `wiki-drilldown`**（`.github/skills/wiki-drilldown/SKILL.md`，含六步流程 + 踩坑 + 自查清单）。要点：
