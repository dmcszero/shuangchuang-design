...
| 2026-09-15 | v1.0.3 ✅ | **发布前可用性调整 + 内嵌使用说明**：① 右侧「详情」「缺口与待确认」两卡由固定 `max-height:300px` 改为**按视口高度平分**（`.side` 定高 `calc(100vh - 34px)`，两 `.box` 用 `flex:1`，`.box-bd` 去掉 max-height）——原来两卡都太矮，看长内容要反复滚；② 工具栏「批注」组里的「怎么收集」升级为**「使用说明」弹层**（`#helpModal`），四节：页面怎么看 / 怎么提意见（批注）与回收 / 「打开 demo ↗」按钮说明 / 三个别误会的地方 —— **面向拿到链接的外部读者**，对方打开页面即可自助，不必口头解释；③ 说明里显式写清三件最容易出事的：「批注只存在你自己的浏览器里，写完必须点导出」「demo 按钮需本机跑 3000 端口，未跑会置灰」「跳 demo 后需先登录」。探针 59 → **63 项全通过**。 |
| 2026-09-15 | v1.0.2 ✅ | **批注出入口提到工具栏（可用性修复）**：导出/导入原先只存在于「详情面板底部的批注区块」——**必须先用鼠标选中某个块、再把右栏拉到底**才看得见，未选中对象时页面上根本没有出口（实测反馈「网页上找不到导出按钮」）。现将「批注」组（`导出（N）` / `导入` / `怎么收集`）固定到**顶部工具栏**，全局可见、不依赖选中状态；详情块里那一份保留（就近操作），两处共用同一套 handler。同时：① 导入改为**动态创建 file input**（不再依赖页面上已有的隐藏 `[data-ann-file]`，工具栏与详情块共用一条通路）；② 导出文件名带**作者 + 日期**（`annotations-庄冉-20260915.json`），避免多人导出后一堆同名文件；③ 新增「怎么收集」按钮，一键给出多人闭环说明（写 → 导出 → 发回收集人 → 导入合并）。探针 54 → **59 项全通过**。 |
| 2026-09-15 | v1.0.1 ✅ | **footer「issues」表格行内批注（§11.1 补）**：①表格新增「决策批注」列——已批注行直读 `💬n` 徽标 + 最近决策 + 作者时间 + 未决数，行尾「批注」按钮**就地展开完整表单**（与详情块同构）；②**数据同源**——与详情块读写同一个 `ANN`，写入经 `refreshAnnViews()` 统一刷新画布 / 缺口列表 / 详情 / 当前表格（双向联通已进探针断言）；③批注表单改 `data-*` 属性 + 就近查找（原用 `id`，两处同时渲染会冲突），新增 `ST.annDraft` 草稿缓存（重渲染不丢已输入的字）与 `ST.tabAnnOpen` 展开态；④**修上一版遗留缺陷**：`issueDetail()` 的「通俗版」分支漏挂 `annBlock`，导致 issue 详情里根本没有批注区块（`raw` 分支有，默认显示的 `plain` 分支没有）——由探针新增断言发现；⑤表格上方加统计条「共 N 条问题 · 已批注 M 条 · 未决 K 条」。探针 42 项 → **54 项全通过**。 |
...
- **入口有两处、数据只有一份**（2026-09-15 补）：① 详情面板底部（node / issue / edge 任意对象）；② footer「issues」表格的**行内展开表单**（点行尾「批注」按钮展开，行内直接显示 `💬n` 徽标 + 最近决策 + 作者时间 + 未决数）。两处读写**同一个 `ANN`**，写入后统一经 `refreshAnnViews()` 刷新画布徽标 / 缺口列表 / 详情 / 当前表格。
  - **表单控件一律用 `data-*` 属性 + 就近查找，不用 `id`** —— 两处会同时渲染（`id` 必然冲突）。
  - 未保存的输入存 `ST.annDraft`（key = `kind::id`），任何重渲染（切页签 / 切批注状态 / 保存刷新）都**不丢已输入的字**；行内展开态存 `ST.tabAnnOpen`。
